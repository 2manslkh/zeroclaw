...
# Prepare data dirs (no baked config) + tiny entry wrapper
RUN mkdir -p /zeroclaw-data/.zeroclaw /zeroclaw-data/workspace \
 && printf '%s\n' \
    '#!/usr/bin/env sh' \
    'set -eu' \
    '# Always clear any stale config so env vars take precedence' \
...
RUN apt-get update && apt-get install -y ca-certificates curl tini && rm -rf /var/lib/apt/lists/* \
...
# Start gateway by default; override to "daemon" for Telegram
